---
title: "CLI & binaries reference"
description: "Neksur ships a single binary for the server plus an operator CLI and a few focused tools. This page is the reference for all of them."
---
...
Exit `0` = compiles cleanly; `1` = CEL error / undeclared binding; `2` = file/usage error. See [Author Access policies](/guides/author-access-policies/).
...
`--dialect` is `cube` (the supported OSI dialect). `--socket` overrides the compiler sidecar socket (`NEKSUR_COMPILER_SOCKET`, default `/var/run/neksur/compiler.sock`). See [Author semantic metrics](/guides/author-semantic-metrics/).
...
| `neksur-verify-chain` | Offline audit-chain verification. | See [Compliance and audit](/guides/compliance-and-audit/). Flags: `--tenant-uuid`, `--database-url`, `--kms-key-arn` or `--public-key-pem`, `--table audit_log\|audit_events`, `--output text\|json`, `--from-id`/`--to-id`. |
...
- [Deployment runbook](/operations/deploy/) — environment variables, HA, backup/DR.
- [Policy language reference](/reference/policy-language/)
