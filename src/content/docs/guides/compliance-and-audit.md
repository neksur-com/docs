...
description: "The Prove job is about defensible evidence: that every Contract was honored, who did what, and that the record hasn't been tampered with. Neksur produces…"
---
...
Exit `0` = chain intact and all signatures valid; non-zero = a break or invalid signature, with the offending row identified. You can scope a range with `--from-id` / `--to-id`, verify `audit_log` or `audit_events`, and verify with either a KMS key or an exported ECDSA public key (`--public-key-pem`). See the [CLI reference](/reference/cli/#neksur-verify-chain).
...
- *Where does this PII propagate, and is every downstream Contract covering it?* — the `pii_propagation` view (also available [to AI agents over MCP](/guides/ai-agents-mcp/)).
