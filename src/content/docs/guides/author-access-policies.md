---
title: "Author Access policies (CEL)"
description: "The Access dimension of a Contract is expressed as policies in CEL (Common Expression Language). A policy is a single boolean expression: it returns true…"
---
...
Row filters and column masks are the ones compiled into the **read path** ([Trino / BI](/guides/connect-read-path/)); the rest gate the **write path** at the gateway.
...
Policies live in the **Access** dimension of a [Data Contract](/concepts/data-contract/). Author and attach them in the web console's Access tab, then advance the Contract through its [lifecycle](/guides/author-and-ship-a-contract/) so they deploy to the gateway and the read path together.

## See also

- [Policy language reference](/reference/policy-language/) — the full binding/field/function catalog.
