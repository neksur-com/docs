...
- **[Meaning](./dimensions.md#meaning)** — what the data *means*: metric and dimension semantics that compile to bit-identical results across every engine.
- **[Access](./dimensions.md#access)** — who may see *what*: row filters, column masks, RBAC, and ABAC, enforced identically on read and write.
- **[State](./dimensions.md#state)** — which *version* of the data everyone sees: snapshot pinning, schema versioning, write-conflict policy, partition-spec evolution, and compaction coordination.

These dimensions are co-equal and coupled — see [Meaning, Access, State](./dimensions.md).

## One governed lifecycle

A Contract is not edited in place on a live system. It moves through a state machine — `draft → review → compile → deploy → active → audit` — with sign-offs at review and rollback paths at every stage. See [The Contract lifecycle](./lifecycle.md).
