...
A [Data Contract](./data-contract.md) is enforced at **several coordinated points** in the data path. Each point catches a different class of risk and fails in a different way; defense-in-depth across all of them is the design point, because no single interception can cover the whole surface.
...
Both paths are proven byte-identical by a CI parity gate, and both are fail-closed: a failed policy fetch aborts the write. This is a Defense-in-Depth edition capability — see [Editions and tiers](./editions.md).
...
- [Editions and tiers](./editions.md)
- [Architecture Overview](../architecture/overview.md) — how these points are wired in code.
