# Author semantic metrics (Meaning)
...
A `time` dimension with a `time_grain_def` unlocks time-intelligence operators (year-to-date, quarter-to-date, prior-period) at query time, compiled via a time-spine join. Fiscal calendars (4-4-5, 4-5-4, etc.) are configured per tenant — see the [CLI reference](../reference/cli.md#fiscal-calendar-provision) or **Settings → Fiscal calendars** in the console.
...
Once a model is active, query it through any read transport — a metric query over `pgwire`, XMLA from Excel/Power BI, or the REST/GraphQL metric endpoints. Whichever engine ultimately runs the SQL, the metric compiles to the same logical computation, and the [Access](./author-access-policies.md) row filters / column masks of the governing Contract still apply.

## Attach Meaning to a Contract

Metrics and dimensions are the **Meaning** dimension of a [Data Contract](../concepts/data-contract.md). Author them in the Contract's Meaning tab, then ship the Contract through its [lifecycle](./author-and-ship-a-contract.md).

## See also

- [Connect Trino and BI tools (read path)](./connect-read-path.md)
- [Author and ship a Data Contract](./author-and-ship-a-contract.md)
- [Meaning, Access, State](../concepts/dimensions.md)
